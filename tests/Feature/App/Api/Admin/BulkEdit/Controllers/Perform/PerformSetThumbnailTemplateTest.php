...
class PerformSetThumbnailTemplateTest extends ControllerTestCase
{
    ...
    /** @test */
    public function can_change_thumbnail_template()
    {
        $displayTemplates = DisplayTemplate::factory(2)->create();
        $products = Product::factory(10)->create();
        foreach($products as $product)
        {
            ProductSettings::factory()->create(['product_id'=>$product->id,'product_thumbnail_template'=>$displayTemplates[0]->id]);
        }
        $this->postJson(
            route('admin.bulk-edit-perform.store'),
            [
                'action_name' => ActionList::SET_THUMBNAIL_TEMPLATE,
                'template_id' => $displayTemplates[1]->id,
                'ids' => $products->pluck('id')->toArray()
            ]
        )
            ->assertOk();
        $this->assertEquals($displayTemplates[1]->id, ProductSettings::first()->product_thumbnail_template);
    }
