...
class PerformUnPublishedOnSiteTest extends ControllerTestCase
{
    ...
    public function can_published_on_site()
    {
        ...
        {
            ProductPricing::factory()->create(['product_id'=>$product->id,'status'=>1,'site_id'=>$site->id]);
        }
        $this->postJson(
            route('admin.bulk-edit-perform.store'),
            [
                'action_name' => ActionList::UNPUBLISH_ON_SITE,
                'site_id' => $site->id,
                'ids' => $products->pluck('id')->toArray()
            ]
        )
            ->assertOk();
        $this->assertEquals(0, ProductPricing::first()->status);
    }
